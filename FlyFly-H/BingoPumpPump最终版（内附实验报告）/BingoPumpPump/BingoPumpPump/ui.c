...
void GamePaint(HWND hwnd, int a)
{
	...
	hpenboundary = CreatePen(PS_SOLID, 2.3, RGB(0, 0, 0));
	...
	if (a == 1)
	{
		...
		MoveToEx(hdcmem, point1.y*FRUIT_PIXEL +65, point1.x*FRUIT_PIXEL + 230, NULL);
		LineTo(hdcmem, point1.y*FRUIT_PIXEL+65, point1.x*FRUIT_PIXEL + 230 + FRUIT_PIXEL);
		LineTo(hdcmem, point1.y*FRUIT_PIXEL + 65 + FRUIT_PIXEL, point1.x*FRUIT_PIXEL + 230 + FRUIT_PIXEL);
		LineTo(hdcmem, point1.y*FRUIT_PIXEL + 65 + FRUIT_PIXEL, point1.x*FRUIT_PIXEL + 230);
		LineTo(hdcmem, point1.y*FRUIT_PIXEL + 65, point1.x*FRUIT_PIXEL + 230);
	}
	if (a == 2)
	{
		SelectObject(hdcmem, hpenboundary);
		MoveToEx(hdcmem, point1.y*FRUIT_PIXEL + 65, point1.x*FRUIT_PIXEL + 228, NULL);
		LineTo(hdcmem, point1.y*FRUIT_PIXEL + 65, point1.x*FRUIT_PIXEL + 228 + FRUIT_PIXEL);
		LineTo(hdcmem, point1.y*FRUIT_PIXEL + 65 + FRUIT_PIXEL, point1.x*FRUIT_PIXEL + 228 + FRUIT_PIXEL);
		LineTo(hdcmem, point1.y*FRUIT_PIXEL + 65 + FRUIT_PIXEL, point1.x*FRUIT_PIXEL + 228);
		LineTo(hdcmem, point1.y*FRUIT_PIXEL + 65, point1.x*FRUIT_PIXEL + 228);

		SelectObject(hdcmem, hpenboundary);
		MoveToEx(hdcmem, point2.y*FRUIT_PIXEL + 65, point2.x*FRUIT_PIXEL + 228, NULL);
		LineTo(hdcmem, point2.y*FRUIT_PIXEL + 65, point2.x*FRUIT_PIXEL + 228 + FRUIT_PIXEL);
		LineTo(hdcmem, point2.y*FRUIT_PIXEL + 65 + FRUIT_PIXEL, point2.x*FRUIT_PIXEL + 228 + FRUIT_PIXEL);
		LineTo(hdcmem, point2.y*FRUIT_PIXEL + 65 + FRUIT_PIXEL, point2.x*FRUIT_PIXEL + 228);
		LineTo(hdcmem, point2.y*FRUIT_PIXEL + 65, point2.x*FRUIT_PIXEL + 228);
	}
	// 在内存DC中画完，一次输出的窗口DC上。
...
LONG CALLBACK MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		if (wParam && MK_LBUTTON&&flag==0)
		{
			x = LOWORD(lParam);
		    y = HIWORD(lParam);
			point1.x = (y - 230) / FRUIT_PIXEL;
			point1.y = (x - 65) / FRUIT_PIXEL;
			flag++;
			GamePaint(hwnd, 1);
		}
		else if (wParam && MK_LBUTTON&&flag == 1)
		{
			x = LOWORD(lParam);
			y = HIWORD(lParam);
			point2.x = (y - 230) / FRUIT_PIXEL;
			point2.y = (x - 65) / FRUIT_PIXEL;
		...
		if (flag == 2)
		{
				...
				point1.y == point2.y&&abs(point1.x - point2.x) == 1)
			{
				...
				if (removenumber[level] < 0)
				{
					removenumber[level] = 0;
				}
				GamePaint(hwnd, 0);
				Sleep(300);
				if (f == 0)
